...
% Input the distributed load. 
load distributedLoad.txt;
Fequiva_stack_local=zeros(nele,12);
for i=1:nele
    qx=distributedLoad(i,1);
    qy=distributedLoad(i,2);
    qz=distributedLoad(i,3);
    Fequiva_stack_local(i,:)=[-qx*Length(i)/2;  -qy*Length(i)/2;   -qz*Length(i)/2; ...
                            0;          qz*Length(i)^2/12;    -qy*Length(i)^2/12; ...
                -qx*Length(i)/2;       -qy*Length(i)/2;       -qz*Length(i)/2; ...
                            0;      -qz*Length(i)^2/12;     qy*Length(i)^2/12];
end

% Transform the equivalent node force from local to global coordinate system.
Fequiva_stack_global=zeros(nele,12);
for i=1:nele
    Fequiva_stack_global(i,:)=squeeze(gamma_stack_local(i,:,:))'*Fequiva_stack_local(i,:)';
end
% Assembly the equivalent node force
Fequiva_global=zeros(ndof,1);
for i=1:nele
    Fequiva_global(mem_id(i,1:12),1)=Fequiva_global(mem_id(i,1:12),1)+Fequiva_stack_global(i,:)';
end

Fequiva_free=Fequiva_global(free_dof);
Fequiva_fixed=Fequiva_global(fixed_dof);
% 


defl=K_ff\(P_free+Fequiva_free);
...
for i=1:nele
    ...
    for j=1:12
        % When we have distributed load, 
        % we should add the equivalent node force to element force.
        ELE_FOR(i,j)=element_force_local(j)+Fequiva_stack_local(i,j); 
    end
end
display(ELE_FOR)
